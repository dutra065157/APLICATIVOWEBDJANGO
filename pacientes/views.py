    ...
    if request.method == "GET":
        ...
        if len(nome.strip()) == 0 or not foto:
            messages.add_message(request, constants.ERROR,
                                 'O campo nome e foto são obrigatórios')
...
def paciente_view(request, id):
    ...
    if request.method == "GET":
        ...
        tuple_grafico = ([str(i.data) for i in consultas],
        ...
        messages.add_message(request, constants.SUCCESS,
                             'Registro de consulta adicionado com sucesso.')
...
def consulta_publica(request, id):
    # Usa get_object_or_404 para evitar erro de DoesNotExist
    consulta = get_object_or_404(Consultas, id=id)
    print(consulta.link_publico)

    # Se o paciente não estiver com pagamento em dia, retorna erro 404
    if not consulta.paciente.pagamento_em_dia:
        raise Http404()

    # Retorna a página de consulta pública
